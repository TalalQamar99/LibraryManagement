...
#include<windows.h>
using namespace std;
string user1 = "Haider Ali", password1 = "haider";			//global variables for library member "Haider Ali"
string user2 = "Farooq Saeed", password2 = "farooq";			//global variables for library member "Farooq Saeed"
string user3 = "Sohaib Farooqi", password3 = "sohaib";		//global variables for library member "sohaib Farooqi"
...
public:
	...
	void display_all()												//member function to display all books of a category
	{
		...
		while (!displayfile.eof())							//display all books until end of file
		{
			getline(displayfile, title, ',');
...
public:
	void add_book()												//member function to add a new book to database
	{
		...
		if (choice == "Electrical" || choice == "Humanities" || choice == "Basic Sciences" || choice == "DOC" || choice == "Journals and Magazines")
		{
			...
			addfile << endl << title << "," << author << "," << ref_id << "," << av_status;
			addfile.close();
